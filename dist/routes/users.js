...
router.post("/", async (req, res) => {
    try {
        ...
        const { name, email, password, role, profileImageUrl, title, phone, clientIds, } = req.body;
        ...
        const user = await prisma.user.create({
            data: {
                ...
                profileImageUrl: (profileImageUrl === null || profileImageUrl === void 0 ? void 0 : profileImageUrl.trim()) || null,
        ...
        if (normalizedRole === "care_manager" &&
...
router.patch("/:id", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        if (!req.user)
            return res.status(401).json({ error: "Unauthorized" });
        const { id } = req.params;
        const { name, email, role, profileImageUrl, title, phone, } = req.body;
        const user = await prisma.user.findUnique({
            where: { id },
        });
        if (!user || user.orgId !== req.user.orgId) {
...
router.delete("/:id", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        ...
        if (user._count.clients > 0) {
            return res.status(400).json({
                error: "This care manager still has assigned clients. Reassign or remove clients before deleting.",
            });
        }
 ...
 */
router.get("/:id/metrics", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        if (!req.user)
            return res.status(401).json({ error: "Unauthorized" });
        const { id } = req.params;
        ...
        const billableRatio = last30Hours > 0
 ...
 */
router.get("/:id/recent-activities", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        if (!req.user)
            return res.status(401).json({ error: "Unauthorized" });
        const { id } = req.params;
